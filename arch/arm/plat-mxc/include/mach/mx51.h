...
#define VA(x)				_AT(void __iomem *, x)

/*
 * boot ROM
 */
#define ROM_BASE_ADDR			UL(0x00000000)
#define ROM_BASE_ADDR_VIRT		VA(0xFA300000)
#define ROM_SIZE			0x1000

/*
 * IRAM
 */
#define IRAM_BASE_ADDR			UL(0x1FFE0000)	/* internal ram */
#define IRAM_BASE_ADDR_VIRT		VA(0xFA3E0000)
#define IRAM_PARTITIONS			16
#define IRAM_PARTITIONS_TO1		12
#define IRAM_SIZE			(IRAM_PARTITIONS * SZ_8K) /* 128 KiB */
...
#if (IRAM_SIZE < (SDMA_IRAM_SIZE + SND_IRAM_SIZE + VPU_IRAM_SIZE + \
	SCC_IRAM_SIZE))
...
#define SND_IRAM_BASE_ADDR		(VPU_IRAM_BASE_ADDR - SND_IRAM_SIZE)
#define SDMA_IRAM_BASE_ADDR		(SND_IRAM_BASE_ADDR - SDMA_IRAM_SIZE)
#define IDLE_IRAM_BASE_ADDR		(SDMA_IRAM_BASE_ADDR - SZ_4K)
#define SUSPEND_IRAM_BASE_ADDR		(IDLE_IRAM_BASE_ADDR - SZ_4K)
/*
 * NFC
 */
#define NFC_AXI_BASE_ADDR		UL(0xCFFF0000)	/* NAND flash AXI */
#define NFC_AXI_BASE_ADDR_VIRT		VA(0xF9000000)
...
#define GPU2D_BASE_ADDR			UL(0xD0000000)

#define TZIC_TO1_BASE_ADDR		UL(0x8FFFC000)
#define TZIC_TO2_BASE_ADDR		UL(0xE0003000)
#define TZIC_BASE_ADDR_VIRT		VA(0xFA100000)
#define TZIC_SIZE			SZ_4K
#define TZIC_TO1_SIZE			TZIC_SIZE
#define TZIC_TO2_SIZE			TZIC_SIZE

#define DEBUG_BASE_ADDR			UL(0x60000000)
#define DEBUG_BASE_ADDR_VIRT		VA(0xFA200000)
...
#define SPBA0_BASE_ADDR_VIRT		VA(0xFB100000)
...
#define AIPS1_BASE_ADDR_VIRT		VA(0xFB000000)
...
#define MX51_WDOG_BASE_ADDR		WDOG1_BASE_ADDR
...
#define AIPS2_BASE_ADDR_VIRT		VA(0xFB200000)
...
#define IIM_BASE_ADDR_VIRT		VA(0xFB300000)
#define IIM_SIZE			SZ_4K
 ...
 * it returns NULL
 */
#define MX5_IO_ADDRESS(x)					\
	(__in_range(x, IRAM) ? IRAM_IO_ADDRESS(x) :		\
	__in_range(x, TZIC_TO1) ? TZIC_TO1_IO_ADDRESS(x) :	\
	__in_range(x, TZIC_TO2) ? TZIC_TO2_IO_ADDRESS(x) :	\
	__in_range(x, DEBUG) ? DEBUG_IO_ADDRESS(x) :		\
	__in_range(x, SPBA0) ? SPBA0_IO_ADDRESS(x) :		\
	__in_range(x, AIPS1) ? AIPS1_IO_ADDRESS(x) :		\
	__in_range(x, AIPS2) ? AIPS2_IO_ADDRESS(x) :		\
	__in_range(x, ROM) ? ROM_IO_ADDRESS(x) :		\
	__in_range(x, IIM) ? IIM_IO_ADDRESS(x) :		\
	NULL)

#if 0
	__in_range(x, NFC_AXI) ? NFC_AXI_IO_ADDRESS(x) :	\
...
#define TZIC_TO1_IO_ADDRESS(x)  \
	(((x) - TZIC_TO1_BASE_ADDR) + TZIC_BASE_ADDR_VIRT)

#define TZIC_TO2_IO_ADDRESS(x)  \
	(((x) - TZIC_TO2_BASE_ADDR) + TZIC_BASE_ADDR_VIRT)
...
#define NFC_AXI_IO_ADDRESS(x) \
	(((x) - NFC_AXI_BASE_ADDR) + NFC_AXI_BASE_ADDR_VIRT)

#define ROM_IO_ADDRESS(x)  \
	(((x) - ROM_BASE_ADDR) + ROM_BASE_ADDR_VIRT)

#define IIM_IO_ADDRESS(x)  \
	(((x) - IIM_BASE_ADDR) + IIM_BASE_ADDR_VIRT)
 ...
 */
#define DMA_REQ_SSI3_TX0		47
#define DMA_REQ_SSI3_RX0		46
...
#define DMA_REQ_SSI3_TX1		37
#define DMA_REQ_IPU			36
#define DMA_REQ_SSI3_RX1		35
...
#define MX51_CHIP_REV_1_0		0x00
#define MX51_CHIP_REV_2_0		0x20
#define MX51_CHIP_REV_3_0		0x30

#ifndef __ASSEMBLY__
extern int mx51_revision(void);
extern void mxc_arch_reset_init(void __iomem *base);
#endif
