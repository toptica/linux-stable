...
struct vpu_mem_desc {
	size_t size;
	dma_addr_t phy_addr;
	void __iomem *cpu_addr;		/* cpu address to free the dma mem */
	void __iomem *virt_uaddr;	/* virtual user space address */
};
...
int vl2cc_init(unsigned long vl2cc_hw_base, size_t size);
