/*
 * arch/arm/mach-mx2/mle_II_mainboard.c
 *
 * Copyright (C) 2010 Arno Euteneuer <arno.euteneuer@toptica.com>
 * Copyright (C) 2014 Toptica AG/Mixed Mode GmbH/Martin Hofherr
 *
...
/*
 * UART configuration for MLE_II
 *
 * - only RxD and TxD lines are used
 */

//The original macro in iomux-mx25.h appears to be wrong:
//The refrence manual claims that CSI_D3 can be configured 
//as the UART5_TXD. iomux-mx25.h says ECB...
//#define MX25_PAD_UART5_TXD	IOMUX_PAD(0x31c, 0x0124, 0x01, 0, 0, NO_PAD_CTRL)

struct uart_pad_desc{
	int uart_dev_id;
	struct pad_desc pads[2];
};

static struct uart_pad_desc laserbox_uart_pads[] = {
	{	
		.uart_dev_id = 0,
		.pads = {
...
static int laserbox_uart_init(struct platform_device *pdev)
{
	...
	for(i=0;i<ARRAY_SIZE(laserbox_uart_pads); i++) {
		if (pdev->id == laserbox_uart_pads[i].uart_dev_id) {
			printk("%s: found uart_dev_id %d for pdev->id %d\n", 
				__FUNCTION__, laserbox_uart_pads[i].uart_dev_id, pdev->id);
			return mxc_iomux_v3_setup_multiple_pads(laserbox_uart_pads[i].pads, 
				ARRAY_SIZE(laserbox_uart_pads[i].pads));
		}
	}
...
static void laserbox_uart_exit(struct platform_device *pdev)
{
	...
	for(i=0;i<ARRAY_SIZE(laserbox_uart_pads);i++) {
		if(pdev->id == laserbox_uart_pads[i].uart_dev_id) {
			return mxc_iomux_v3_release_multiple_pads(laserbox_uart_pads[i].pads, 
			ARRAY_SIZE(laserbox_uart_pads[i].pads));
		}
	}
}
...
/*
 * SPI configuration for MLE_II 
 *
 * CSPI1 is used for MEMS mirror & A/D converters
 * CSPI2 is used for Laser drivers
 *
 * The pins for CSPI1 are configured by default for SPI operation.
 *
 */
...
#define CSPI1_SYSEEP_SSEL (GPIO_PORTC | 15)


static struct pad_desc mrcm_pads[] = {
	MX25_PAD_D15__GPIO_4_5,	//FCLK_MEMS
};

/* LTC2449 A/D converter platform data */
struct ltc2449_platformdata {
	char *name;
	u8 isr_mask;
};

struct ltc2449_platformdata adcmon_pdata = {
	.name = "adc_mon",
	.isr_mask = (1 << 0),
};

struct ltc2449_platformdata adc1_pdata = {
	.name = "adc_1",
	.isr_mask = (1 << 1),
};

struct ltc2449_platformdata adc_a_pdata = {
	.name = "adc_a",
	.isr_mask = (1 << 3),
};

struct ltc2449_platformdata adc_b_pdata = {
	.name = "adc_b",
	.isr_mask = (1 << 2),
};

#if 0
// No more available with HW revision 0.3.0
...
static struct spi_eeprom mti25LC256 = {
        .byte_len       = SZ_32K,
        .name           = "at25640a",
        .page_size      = 32,
        .flags          = EE_ADDR2,
...
static struct pad_desc cspi_pads[] = {
	//CSPI1 PADS
...
static int mxcspi1_available_cs[1] = {-4};
...
static int mxcspi2_available_cs[1] = {-4};
...
/* Chip select multiplexer driver setup. */

static int register_mleII_csmux_gpios(int *gpios, int size)
{
	int i, ret;

	for(i=0;i<size;i++) {
		ret = gpio_request(gpios[i], "SYSEEP SSEL");
		if(ret) {
			printk(KERN_ERR"%s: request for SYSEEP CS gpio %d failed!",
				__FUNCTION__, gpios[i]);
			return ret;
		}

		ret = gpio_direction_output(CSPI1_SYSEEP_SSEL, 1);
		if(ret) {
			printk(KERN_ERR"%s: could not set SYSEEP CS %d gpio as output!\n",
				__FUNCTION__, gpios[i]);
			return ret;
		}
	}
...
static struct resource csmux_resources[] = {
	{
	       .start = 0x00000000,
	       .end = 0x00000000,
...
static int csmux0_gpios[] = {CSPI1_SYSEEP_SSEL};

#define CSMUX0_BUSNO 3

//CSMUX0 FPGA bit postions
#define CSMUX0_FPGA_ADC_MON 0x01
#define CSMUX0_FPGA_ADC_1   0x02
#define CSMUX0_FPGA_ADC_A   0x04
#define CSMUX0_FPGA_ADC_B   0x08
#define CSMUX0_FPGA_EEPROM0 0xF0
#define CSMUX0_FPGA_EEPROM1 0xE0
#define CSMUX0_FPGA_EEPROM2 0xD0
#define CSMUX0_FPGA_EEPROM3 0xC0
#define CSMUX0_FPGA_EEPROM4 0xB0
#define CSMUX0_FPGA_EEPROM5 0xA0
#define CSMUX0_FPGA_EEPROM6 0x90
#define CSMUX0_FPGA_EEPROM7 0x80


#define FPGA_CS0(x) ((ARRAY_SIZE(csmux0_gpios))+(x))

static int csmux0_fpga[] = {CSMUX0_FPGA_ADC_MON, CSMUX0_FPGA_ADC_1, 
	CSMUX0_FPGA_ADC_A,CSMUX0_FPGA_ADC_B, CSMUX0_FPGA_EEPROM0, 
	CSMUX0_FPGA_EEPROM1,CSMUX0_FPGA_EEPROM2, CSMUX0_FPGA_EEPROM3,
	CSMUX0_FPGA_EEPROM4,CSMUX0_FPGA_EEPROM5, CSMUX0_FPGA_EEPROM6,
	CSMUX0_FPGA_EEPROM7,
};

static struct mle_ii_cs_mux_pdata mleII_csmux0_pdata = {
	.bus_num = CSMUX0_BUSNO,
	.ngpio_cs = ARRAY_SIZE(csmux0_gpios),
	.gpio_cs = csmux0_gpios,

	.nfpga_cs = ARRAY_SIZE(csmux0_fpga),
	.fpga_cs = csmux0_fpga,
	.reset_val = 0x7F,
	.fpga_cs_base = 0x00FA,
	.fpga_cs_size = 0x02,
...
struct platform_device mleII_csmux0 = {
	.name = "mle_ii_cs_mux",
	.id = CSMUX0_BUSNO, 
	.num_resources = ARRAY_SIZE(csmux_resources),
	.resource = csmux_resources,
};

static int csmux1_gpios[] = {};

#define CSMUX1_BUSNO 4

//CSMUX1 FPGA bit masks
#define CSMUX1_FPGA_LD1	0x01
#define CSMUX1_FPGA_LD2	0x02
#define CSMUX1_FPGA_LD3	0x04
#define CSMUX1_FPGA_LD4	0x08
#define CSMUX1_FPGA_LD5	0x10
#define CSMUX1_FPGA_LD6	0x20
#define CSMUX1_FPGA_LD7	0x40
#define CSMUX1_FPGA_LD8	0x80

#define FPGA_CS1(x) ((ARRAY_SIZE(csmux1_gpios))+(x))

static int csmux1_fpga[] = {CSMUX1_FPGA_LD1, CSMUX1_FPGA_LD2, CSMUX1_FPGA_LD3,
		CSMUX1_FPGA_LD4, CSMUX1_FPGA_LD5, CSMUX1_FPGA_LD6, CSMUX1_FPGA_LD7, 
		CSMUX1_FPGA_LD8};



static struct mle_ii_cs_mux_pdata mleII_csmux1_pdata = {
	.bus_num = CSMUX1_BUSNO,
	.ngpio_cs = ARRAY_SIZE(csmux1_gpios),
	.gpio_cs = csmux1_gpios,

	.nfpga_cs = ARRAY_SIZE(csmux1_fpga),
	.fpga_cs = csmux1_fpga,
	.reset_val = 0x00,
	.fpga_cs_base = 0x00FC,
	.fpga_cs_size = 0x02,
...
struct platform_device mleII_csmux1 = {
	.name = "mle_ii_cs_mux",
	.id = CSMUX1_BUSNO,
	.num_resources = ARRAY_SIZE(csmux_resources),
	.resource = csmux_resources,
...
/* The number of gpios and native chip selects is not sufficient for all SPI devices in the 
 * MLE II. Therefore we need a chip select multiplexer in the FPGA. The required chipselect
 * is set in an address register in the FPGA and the native chip select is routed from the 
 * FPGA to the device on the board.
 * Note: The iChrome SLE maximum SPI frequency is 1MHz, faster frequency will disturb the bus
 *       and ruin the SPI communication completely.
 */

static struct spi_board_info spi_board_info[] __initdata = {
	{
		.modalias	= "mle_ii_csmux",
		.platform_data = (void *) &mleII_csmux0_pdata,
		.mode		= SPI_MODE_0,
	...
	{
		.modalias	= "mle_ii_csmux",
		.platform_data = (void *) &mleII_csmux1_pdata,
		.mode		= SPI_MODE_3,
		.max_speed_hz	= 1 * 1000 * 1000,
		.bus_num	= 1,
		.chip_select	= 0,
	...
	{
		.modalias	= "ltc2449",
		.platform_data 	= (void *) &adcmon_pdata,
		.mode		= SPI_MODE_0,
		.max_speed_hz	= 1 * 1000 * 1000,
		.bus_num	= 3,
		.chip_select	= FPGA_CS0(0),
	},
	{
		.modalias	= "ltc2449",
		.platform_data 	= (void *) &adc1_pdata,
		.mode		= SPI_MODE_0,
		.max_speed_hz	= 1 * 1000 * 1000,
		.bus_num	= 3,
		.chip_select	= FPGA_CS0(1),
	},
	{
		.modalias	= "ltc2449",
		.platform_data 	= (void *) &adc_a_pdata,
		.mode		= SPI_MODE_0,
		.max_speed_hz	= 1 * 1000 * 1000,
		.bus_num	= 3,
		.chip_select	= FPGA_CS0(2),
	},
	{
		.modalias	= "ltc2449",
		.platform_data 	= (void *) &adc_b_pdata,
		.mode		= SPI_MODE_0,
		.max_speed_hz	= 1 * 1000 * 1000,
		.bus_num	= 3,
		.chip_select	= FPGA_CS0(3),
	},

	// System EEPROM on base plate
	{
                .modalias       = "at25",
                .platform_data  = &mti25LC256,
                .max_speed_hz   = 1 * 1000 * 1000,     /* at 3v3 */
                .bus_num        = 3,
                .chip_select    = 0,
                .mode           = SPI_MODE_0, 
        },

	//These are the EEPROMs on the laser modules
	{
                .modalias       = "at25",
                .platform_data  = &mti25LC256,
                .max_speed_hz   = 1 * 1000 * 1000,     /* at 3v3 */
                .bus_num        = 3,
                .chip_select    = FPGA_CS0(4),
                .mode           = SPI_MODE_0, 
        },
	{
                .modalias       = "at25",
	...
	},
	{
		...
		.max_speed_hz	= 1 * 1000 * 1000,
		.bus_num	= 4,
		.chip_select	= FPGA_CS1(7),
	},
	
};

static int register_mrcm_gpios(void)
{
	int ret = 0;

	ret = mxc_iomux_v3_setup_multiple_pads(mrcm_pads, ARRAY_SIZE(mrcm_pads));
	if(ret) {
		printk(KERN_ERR"%s:Failed to setup MEMS mirror pads!\n",
			__FUNCTION__);
		return ret;
	}
...
static int __init laserbox_spi_register(void)
{
	...
	ret = mxc_iomux_v3_setup_multiple_pads(cspi_pads, ARRAY_SIZE(cspi_pads));
	if(ret) {
		printk(KERN_ERR"Failed to setup CSPI1/2 pads!\n");
		return ret;
	}

	ret = register_mleII_csmux_gpios(csmux1_gpios, ARRAY_SIZE(csmux1_gpios));
	if(ret) {
		printk(KERN_ERR"%s: csmux gpio setup failed!\n", __FUNCTION__);
		return ret;
	}

	//Register MEMS mirror
	ret = register_mrcm_gpios();
	if(ret) {
		printk(KERN_ERR"MRCM gpio setup failed!\n");
		return ret;
	}
	
	ret = mxc_register_device(&mleII_csmux0, &mleII_csmux0_pdata);
	if(ret) {
		printk(KERN_ERR"Failed to register MLE II chip select multiplexer 0\n");
		return ret;
	}

	ret = mxc_register_device(&mleII_csmux1, &mleII_csmux1_pdata);
	if(ret) {
		printk(KERN_ERR"Failed to register MLE II chip select multiplexer 1\n");
		return ret;
	}

	ret = mxc_register_device(&mxc_spi_device0, &mxcspi1_data);
	if (ret) {
		printk(KERN_ERR "Failed to register CSPI1 %d\n",ret);
		return ret;
	}

	ret = mxc_register_device(&mxc_spi_device1, &mxcspi2_data);
	if (ret) {
		printk(KERN_ERR "Failed to register CSPI2 %d\n",ret);
		return ret;
	}
...
/* GPIO expanders on the supply board */
static struct pca953x_platform_data pca9557_supply = {
	.gpio_base	= 144,
};

/* GPIO expanders on the base plate to read out the HW information*/
/* So these GPIOs are just used as direction IN*/
static struct pca953x_platform_data pca9557_base_plate = {
	.gpio_base	= 152,
};

/**
 * Platform data for the PCA9551 LED drivers. These drivers are located on the HMI 2
 * board and control the seven segment display and the status leds.
 */
#define PHD_POTI_CS (GPIO_PORTB | 22)
#define POTI1_CS (GPIO_PORTB | 11)
#define POTI2_CS (GPIO_PORTB | 9)
#define POTI3_CS (GPIO_PORTB | 7)
#define POTI4_CS (GPIO_PORTB | 4)
#define POTI5_CS (GPIO_PORTB | 30)
#define POTI6_CS (GPIO_PORTC | 0)
#define POTI7_CS (GPIO_PORTC | 3)
#define POTI8_CS (GPIO_PORTB | 1)

static struct i2c_board_info mle_II_i2c_0_boardinfo[] __initdata = {
	//GPIO expander supply
	{
		I2C_BOARD_INFO("pca9557", 0x1B),
		.platform_data = &pca9557_supply,
	},

	//Base plate HW version information
	{
		I2C_BOARD_INFO("pca9557", 0x1F),
		.platform_data = &pca9557_base_plate,
	},

	/* Laser diode potentiomenter 1 to 7 
	 * The potentiometer get all the same address. Each MCP4451 has an address switch pin,
	 * which can be used to emulate a chip select signal. Thus only the potentiometer whose
	 * "chip select" is low, will have the I2C address specified here.
	 */
	{ 
		I2C_BOARD_INFO("mcp4451", 0x2C),
	},
	{ 
		I2C_BOARD_INFO("mcp4451", 0x2E),
	},
	{ 
		I2C_BOARD_INFO("mcp4451", 0x2F),
	},
	...
	// Temperatur sensor detection unit (camera)
	{
...
int __init mle_II_i2c_add_devices(void)
{
	int ret;

	ret = i2c_register_board_info(0, mle_II_i2c_0_boardinfo,
				      ARRAY_SIZE(mle_II_i2c_0_boardinfo));
	if (ret != 0) {
		printk(KERN_ERR "Failed to register I2C board info: %d\n", ret);
	}

	printk(KERN_ERR"registered %d I2C devices!\n", ARRAY_SIZE(mle_II_i2c_0_boardinfo));

	return ret;
}
device_initcall(mle_II_i2c_add_devices);
...
static struct pad_desc laserbox_fpga_pads[] = {
	/* fpga configuration */
	MX25_PAD_LD2__GPIO_2_17, //nCONFIG
	MX25_PAD_LD4__GPIO_2_19, //CONF_DONE
	MX25_PAD_A17__GPIO_2_3, //nSTATUS
	//MX25_PAD_OE__GPIO_2_14, //FPGA OE
	MX25_PAD_A19__GPIO_2_5, //DCLK
	MX25_PAD_CS0__CS0,
	MX25_PAD_CS1__CS1,
