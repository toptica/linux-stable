...
static void enter_lp_video_mode(void)
{
}

static void exit_lp_video_mode(void)
{
}

static void enter_lp_audio_mode(void)
{
	struct clk *tclk;
	int ret;
	...
	if (IS_ERR(tclk)) {
		printk(KERN_ERR "Failed to get DDR clock: %ld\n",
			PTR_ERR(tclk));
		return;
	}
