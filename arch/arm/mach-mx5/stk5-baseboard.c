 ...
 */
static const uint32_t tx51_kpd_keycodes[] = {
	/* specify your keymap with KEY(row, col, keycode), */
	KEY(0, 0, KEY_POWER),
};

static struct matrix_keymap_data tx51_keymap_data = {
	.keymap = tx51_kpd_keycodes,
	.keymap_size = ARRAY_SIZE(tx51_kpd_keycodes),
};

static struct matrix_keypad_platform_data tx51_keypad_data = {
	.keymap_data = &tx51_keymap_data,
	.wakeup = 1,
};

static struct mx51_pad_desc tx51_keypad_pads[] __initdata = {
	MX51_PIN_KEY_ROW0__KPP_ROW0,
	MX51_PIN_KEY_ROW1__KPP_ROW1,
	MX51_PIN_KEY_ROW2__KPP_ROW2,
	MX51_PIN_KEY_ROW3__KPP_ROW3,

	MX51_PIN_KEY_COL0__KPP_COL0,
	MX51_PIN_KEY_COL1__KPP_COL1,
	MX51_PIN_KEY_COL2__KPP_COL2,
	MX51_PIN_KEY_COL3__KPP_COL3,
	MX51_PIN_KEY_COL4__KPP_COL4,
	MX51_PIN_KEY_COL5__KPP_COL5,
};

static int __init tx51_keypad_init(void)
{
	return mx51_iomux_request_pads(tx51_keypad_pads,
				ARRAY_SIZE(tx51_keypad_pads));
}
device_initcall(tx51_keypad_init);
#endif
			...
			irq_handler_t detect_irq, void *data)
{
	...
	ret = request_irq(irq, detect_irq, IRQF_DISABLED |
...
static struct platform_pwm_backlight_data stk5_backlight_data = {
	...
	.dft_brightness = 50,
	.pwm_period_ns = KHZ2PICOS(1000), /* kHz -> ps is the same as Hz -> ns */
...
#endif
#if defined(CONFIG_KEYBOARD_IMX) || defined(CONFIG_KEYBOARD_IMX_MODULE)
	{ .pdev = &mx51_keypad_device, .pdata = &tx51_keypad_data, },
#endif
};
