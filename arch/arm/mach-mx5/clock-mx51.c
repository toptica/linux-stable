...
static int _clk_parent_set_rate(struct clk *clk, unsigned long rate)
{
	...
	if (ret != 0) {
		printk(KERN_ERR "Failed to set clk %s rate to %lu: %d\n",
			_clk_lookup_name(clk->parent), rate, ret);
	}
	return ret;
}
...
static int _clk_pll_set_rate(struct clk *clk, unsigned long rate)
{
	...
	dp_ctl = __raw_readl(clk->enable_reg + MXC_PLL_DP_CTL);
	/* use dpdck0_2 */
	__raw_writel(dp_ctl | MXC_PLL_DP_CTL_DPDCK0_2_EN, clk->enable_reg + MXC_PLL_DP_CTL);
	pll_hfsm = dp_ctl & MXC_PLL_DP_CTL_HFSM;
	if (pll_hfsm == 0) {
		reg = (mfi << 4) | pdf;
...
err:
	printk(KERN_WARNING "Cannot achieve %lu.%03luMHz clock rate with parent rate %lu.%03luMHz\n",
...
{
	u32 reg = readl(clk->enable_reg + MXC_PLL_DP_CTL);

	reg &= ~MXC_PLL_DP_CTL_REF_CLK_SEL_MASK;
	if (parent == &fpm_clk)
		reg |= MXC_PLL_DP_CTL_REF_CLK_SEL_FPM;
	else if (parent == &osc_clk)
		reg |= MXC_PLL_DP_CTL_REF_CLK_SEL_OSC;
	else
		return -EINVAL;

	writel(reg, clk->enable_reg + MXC_PLL_DP_CTL);
	return 0;
}
...
static void _clk_hsc_disable(struct clk *clk)
{
	...
	/* No handshake with HSC as it's not enabled. */
	reg = __raw_readl(MXC_CCM_CCDR);
	reg |= MXC_CCM_CCDR_HSC_HS_MASK;
	__raw_writel(reg, MXC_CCM_CCDR);
...
static struct clk_lookup lookups[] = {
	...
	_REGISTER_CLOCK("imx-keypad", "kpp", kpp_clk),
