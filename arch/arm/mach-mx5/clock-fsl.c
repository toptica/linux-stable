...
{
	clk->set_rate(NULL, ckil_clk.get_rate() * 512);
...
static void _clk_pll_recalc(struct clk *clk)
{
	...
	ref_clk = 2 * _clk_parent_get_rate(clk);
	if (dbl != 0)
		ref_clk *= 2;
...
static void _clk_ipu_di_recalc(struct clk *clk)
{
	...
	if (mux == 0) {
		...
		rate = _clk_parent_get_rate(clk) / 8;
	} else {
		rate = _clk_parent_get_rate(clk);
	}
}
...
{
	u32 pre, post;
	u32 div = _clk_parent_get_rate(clk) / rate;
...
static unsigned long usb_clk_get_rate(struct clk *clk)
{
	return 60000000;
}
...
int __init mxc_clocks_init(unsigned long ckil, unsigned long osc, unsigned long ckih1, unsigned long ckih2)
{
	...
	ckil_clk.set_rate(NULL, ckil);
	osc_clk.set_rate(NULL, osc);
	ckih_clk.set_rate(NULL, ckih1);
	ckih2_clk.set_rate(NULL, ckih2);
	 ...
	 * at boot time
	 */
	if (pll1_main_clk.get_rate(NULL) <= cpu_wp_tbl[cpu_wp_nr - 1].cpu_rate)
		wp_cnt = 1;
	else if (pll1_main_clk.get_rate(NULL) <= cpu_wp_tbl[1].cpu_rate &&
				pll1_main_clk.get_rate(NULL) > cpu_wp_tbl[2].cpu_rate)
		wp_cnt = cpu_wp_nr - 1;
	else
		wp_cnt = cpu_wp_nr;

	cpu_wp_tbl[0].cpu_rate = pll1_main_clk.get_rate(NULL);
	...
	for (j = 0; j < cpu_wp_nr; j++) {
		if ((ddr_clk.parent == &ddr_hf_clk)) {
			...
			cpu_wp_tbl[j].cpu_podf =
				(pll1_main_clk.get_rate(NULL) / cpu_wp_tbl[j].cpu_rate)
				- 1;
			if (pll1_main_clk.get_rate(NULL)/(cpu_wp_tbl[j].cpu_podf + 1) >
					cpu_wp_tbl[j].cpu_rate) {
				cpu_wp_tbl[j].cpu_podf++;
				cpu_wp_tbl[j].cpu_rate =
					 pll1_main_clk.get_rate(NULL)/
					 (1000 * (cpu_wp_tbl[j].cpu_podf + 1));
				cpu_wp_tbl[j].cpu_rate *= 1000;
			}
			if (pll1_main_clk.get_rate(NULL)/(cpu_wp_tbl[j].cpu_podf + 1) <
						cpu_wp_tbl[j].cpu_rate) {
				cpu_wp_tbl[j].cpu_rate = pll1_main_clk.get_rate(NULL);
			}
		}
	cpu_wp_tbl[j].pll_rate = pll1_main_clk.get_rate(NULL);
	}
	/* Set the current working point. */
	for (i = 0; i < cpu_wp_nr; i++) {
...
static int cpu_clk_set_wp(int wp)
{
	...
	if (ddr_clk.parent == &ddr_hf_clk) {
		...
		cpu_curr_wp = wp;
		cpu_clk.set_rate(NULL, cpu_wp_tbl[wp].cpu_rate);
	} else {
		struct timespec nstimeofday;
		struct timespec curtime;
		...
		pll1_sw_clk.set_rate(NULL, cpu_wp_tbl[wp].cpu_rate);
		pll1_main_clk.set_rate(NULL, pll1_sw_clk.get_rate(NULL));
		cpu_clk.set_rate(NULL, pll1_sw_clk.get_rate(NULL));
	}
