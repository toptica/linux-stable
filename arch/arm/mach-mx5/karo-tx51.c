...
#define FEC_PWR_GPIO		IOMUX_TO_GPIO(MX51_PIN_GPIO1_3)
#define FEC_RST_GPIO		IOMUX_TO_GPIO(MX51_PIN_EIM_A20)
#define FEC_MODE0_GPIO		IOMUX_TO_GPIO(MX51_PIN_NANDF_D9)
#define FEC_MODE1_GPIO		IOMUX_TO_GPIO(MX51_PIN_EIM_EB3)
#define FEC_MODE2_GPIO		IOMUX_TO_GPIO(MX51_PIN_EIM_CS2)
...
static void tx51_fec_inactive(void)
{
	...
	/* Configure mode pins for power down mode */
	gpio_direction_output(FEC_MODE0_GPIO, 0);
	gpio_direction_output(FEC_MODE1_GPIO, 1);
	gpio_direction_output(FEC_MODE2_GPIO, 1);
	/* Release PHY reset */
	...
	if (!gpio_get_value(FEC_PWR_GPIO) ||
		...
		for (i = 0; i < ARRAY_SIZE(tx51_fec_strap_gpios); i++) {
			DBG(0, "%s: Asserting GPIO%d_%d\n", __FUNCTION__,
				...
				tx51_fec_strap_gpios[i] % 32);
			gpio_direction_output(tx51_fec_strap_gpios[i], 1);
		}
		mdelay(22);
