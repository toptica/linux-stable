...
int ipu_select_buffer(ipu_channel_t channel,
...
/* SDC API */
int ipu_sdc_init_panel(ipu_panel_t panel,
			   uint32_t pixel_clk,
			   uint16_t width, uint16_t height,
			   uint32_t pixel_fmt,
...
int ipu_sdc_set_global_alpha(bool enable, uint8_t alpha);
int ipu_sdc_set_color_key(ipu_channel_t channel, bool enable,
			      uint32_t colorKey);
int ipu_sdc_set_brightness(uint8_t value);

int ipu_init_sync_panel(int disp,
			    uint32_t pixel_clk,
			    uint16_t width, uint16_t height,
			    uint32_t pixel_fmt,
...
int ipu_disp_set_window_pos(ipu_channel_t channel, int16_t x_pos,
				int16_t y_pos);
int ipu_disp_set_global_alpha(ipu_channel_t channel, bool enable,
				  uint8_t alpha);
int ipu_disp_set_color_key(ipu_channel_t channel, bool enable,
			       uint32_t colorKey);
...
/* ADC API */
int ipu_adc_write_template(display_port_t disp, uint32_t * pCmd,
			       bool write);

int ipu_adc_set_update_mode(ipu_channel_t channel,
				ipu_adc_update_mode_t mode,
				uint32_t refresh_rate, unsigned long addr,
				uint32_t * size);

int ipu_adc_get_snooping_status(uint32_t * statl, uint32_t * stath);

int ipu_adc_write_cmd(display_port_t disp, cmddata_t type,
			  uint32_t cmd, const uint32_t * params,
			  uint16_t numParams);

int ipu_adc_init_panel(display_port_t disp,
			   uint16_t width, uint16_t height,
			   uint32_t pixel_fmt,
			   uint32_t stride,
...
int ipu_adc_init_ifc_timing(display_port_t disp, bool read,
				uint32_t cycle_time,
				uint32_t up_time,
				uint32_t down_time,
...
int ipu_csi_init_interface(uint16_t width, uint16_t height,
			       uint32_t pixel_fmt, ipu_csi_signal_cfg_t sig);

int ipu_csi_enable_mclk(int src, bool flag, bool wait);
...
int ipu_pf_set_pause_row(uint32_t pause_row);

static inline uint32_t ipu_bytes_per_pixel(uint32_t fmt)
{
	switch (fmt) {
