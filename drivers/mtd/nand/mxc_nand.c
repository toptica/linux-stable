				 ...
				 u_char *read_ecc, u_char *calc_ecc)
{
	...
	if (cpu_is_mx25()) {
		...
		do {
			if ((ecc_status & 0xf) > 4) {
				...
				if (last_bad != host->page_addr >> pg2blk_shift) {
					...
					dev_err(host->dev,
						"HWECC uncorrectable ECC error in block %u page %u subpage %d\n",
		...
		if (((ecc_status & 0x3) == 2) || ((ecc_status >> 2) == 2)) {
			dev_err(host->dev, "HWECC uncorrectable 2-bit ECC error\n");
			return -1;
		}
	}
...
static void mxc_nand_select_chip(struct mtd_info *mtd, int chip)
{
	...
	if (chip > 0) {
		dev_err(host->dev, "Illegal chip select %d\n", chip);
		return;
	}
...
static int mxcnd_scan_bbt(struct mtd_info *mtd)
{
	struct nand_chip *chip = mtd->priv;

	/* mtd->flags is initialized inside nand_scan_tail()
	 * before calling chip->scan_bbt(), but MTD_OOB_WRITEABLE
	 * need to be cleared for i.MX25 since the ECC algorithm
	 * does not allow for arbitrary writes to the OOB area.
	 */
...
static int __init mxcnd_probe(struct platform_device *pdev)
{
	...
	if (err) {
		dev_err(&pdev->dev, "Unable to find any NAND device\n");
		goto escan;
	}
