			...
			const char *buf, size_t count)
{
	...
	/* Write value */
	value = (short) (SENSORS_LIMIT(temp, (LM73_TEMP_MIN*1000),
		(LM73_TEMP_MAX*1000)) * 100 / 3125 ) << 2;
			...
			const char *buf, size_t count)
{
	...
	value = (u8) (SENSORS_LIMIT(temp,11,14) - 11) << 5;
	i2c_smbus_write_byte_data(client, LM73_REG_CTRL, value);
	return count;
}
			 ...
			 char *buf)
{
	...
	u8 temp = ((i2c_smbus_read_byte_data(client,
			LM73_REG_CTRL) & 0x60) >> 5) + 11;
	return sprintf(buf, "%d\n", temp);
}
...
static SENSOR_DEVICE_ATTR(temp1_bits, S_IWUSR | S_IRUGO,
...
static struct attribute *lm73_attributes[] = {
	...
	&sensor_dev_attr_temp1_bits.dev_attr.attr,
