...
#define PP_DUMP(reg)	pr_debug("%s[%08lx]\t = 0x%08x\n", #reg, \
			reg - MX2_IO_ADDRESS(EMMA_PP_BASE_ADDR) + EMMA_PP_BASE_ADDR, __raw_readl(reg))
void pp_dump(void)
{
	PP_DUMP(PP_CNTL);
	PP_DUMP(PP_INTRCNTL);
 ...
 * @param enable	Zero to disable PP, others to enable PP
 * @return	Zero on success, others on failure
 */
int pp_enable(int enable)
{
	return pphw_enable(enable);
}
...
static int pp_initialized;

int pp_init(vout_data *vout)
{
	int ret;

	if (WARN_ON(pp_initialized))
		return -EINVAL;
 ...
 * @return	 Zero on success, others on failure
 */
static int pphw_enable(int enable)
{
	int ret = 0;
...
static void pphw_exit(void)
{
	printk(KERN_DEBUG "%s: Disabling EMMA clk\n", __FUNCTION__);
