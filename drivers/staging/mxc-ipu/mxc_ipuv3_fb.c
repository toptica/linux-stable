...
static int def_vxres;
static int def_vyres;
...
static uint32_t bpp_to_pixfmt(struct fb_info *fbi)
{
	...
	if (fbi->var.nonstd) {
		DBG(0, "%s: pixfmt=%d\n", __FUNCTION__, fbi->var.nonstd);
		return fbi->var.nonstd;
	}

	DBG(0, "%s: bpp=%d\n", __FUNCTION__, fbi->var.bits_per_pixel);
...
static int _setup_disp_channel1(struct fb_info *fbi)
{
	...
	if (mxc_fbi->ipu_ch == MEM_FG_SYNC) {
		...
		for (i = 0; i < num_registered_fb; i++) {
			mxc_fbi_tmp = registered_fb[i]->par;
...
static int mxcfb_set_par(struct fb_info *fbi)
{
	...
	ipu_disable_channel(mxc_fbi->ipu_ch, 1);
	...
	if (mxc_fbi->alpha_chan_en) {
		...
		if ((!mxc_fbi->alpha_phy_addr0 && !mxc_fbi->alpha_phy_addr1) ||
			...
			if (mxc_fbi->alpha_virt_addr0 == NULL ||
				...
				dev_err(fbi->device, "mxcfb: dma alloc for alpha buffer failed\n");
			  ...
			  struct fb_info *fbi_to, bool both_on)
{
	...
	struct mxcfb_info *mxc_fbi_from = fbi->par;
	struct mxcfb_info *mxc_fbi_to = fbi_to->par;

	if (both_on) {
		ipu_disable_channel(mxc_fbi_to->ipu_ch, 1);
...
static int swap_channels(struct fb_info *fbi)
{
	...
	case BOTH_ON:
		...
		_swap_channels(fbi, fbi_to, 1);
		break;
	case SRC_ON:
		/* just switch src */
		_swap_channels(fbi, fbi_to, 0);
		break;
	case TGT_ON:
		/* just switch target */
		_swap_channels(fbi_to, fbi, 0);
		break;
	case BOTH_OFF:
...
static int mxcfb_check_var(struct fb_var_screeninfo *var, struct fb_info *info)
{
	...
	var->width = -1;
	var->grayscale = 0;
	DBG(1, "%s: %dx%d-%d HS:%d:%d:%d VS %d:%d:%d\n", __FUNCTION__,
			...
			if (la.enable) {
				...
				for (i = 0; i < num_registered_fb; i++) {
					...
					if (strcmp(idstr, video_plane_idstr) == 0) {
						((struct mxcfb_info *)(registered_fb[i]->par))->alpha_chan_en = 0;
						break;
					}
				}
			} else
	...
	case MXCFB_SET_LOC_ALP_BUF:
		{
			...
			if (!(((mxc_fbi->ipu_ch == MEM_FG_SYNC) ||
				...
				dev_err(fbi->device,
					"Should use background or overlay framebuffer to set the alpha buffer number\n");
				return -EINVAL;
			}
			...
			if (base != mxc_fbi->alpha_phy_addr0 &&
				...
				dev_err(fbi->device,
					"Wrong alpha buffer physical address %lu\n", base);
				return -EINVAL;
			}
			...
			if (ipu_update_channel_buffer(mxc_fbi->ipu_ch,
				...
				dev_err(fbi->device,
					"Error updating %s SDC alpha buf %d to address=0x%08lX\n",
					fbi->fix.id,
	...
	case MXCFB_SET_OVERLAY_POS:
		{
			...
			struct mxcfb_info *bg_mxc_fbi = NULL;
			int i;

			if (mxc_fbi->ipu_ch != MEM_FG_SYNC) {
				dev_err(fbi->device, "Should use the overlay framebuffer to set the position of the overlay window\n");
				retval = -EINVAL;
				break;
			}
			...
			for (i = 0; i < num_registered_fb; i++) {
				bg_mxc_fbi = registered_fb[i]->par;

				if (bg_mxc_fbi->ipu_ch == MEM_BG_SYNC) {
...
static int mxcfb_blank(int blank, struct fb_info *info)
{
	...
	case FB_BLANK_NORMAL:
		DBG(0, "%s: Disabling IPU channel %d\n", __FUNCTION__,
			mxc_fbi->ipu_ch);
		msleep(100);
		ipu_disable_channel(mxc_fbi->ipu_ch, 1);
		DBG(0, "%s: Shutting down IPU channel %d\n", __FUNCTION__,
			mxc_fbi->ipu_ch);
		ipu_uninit_channel(mxc_fbi->ipu_ch);
		msleep(100);
		break;
	case FB_BLANK_UNBLANK:
		return mxcfb_set_par(info);
	}
	return 0;
}
...
static int mxcfb_map_video_memory(struct fb_info *fbi)
{
	...
	dev_dbg(fbi->device, "allocated fb @ paddr=0x%08lX, size=%u\n",
		(unsigned long)fbi->fix.smem_start, fbi->fix.smem_len);
			      ...
			      struct device_attribute *attr, char *buf)
{
	...
	else if (mxc_fbi->ipu_ch == MEM_FG_SYNC)
		return sprintf(buf, "2-layer-fb-fg\n");
	else if (mxc_fbi->ipu_ch == MEM_DC_SYNC)
			      ...
			      const char *buf, size_t count)
{
	struct fb_info *info = dev_get_drvdata(dev);
	struct mxcfb_info *mxc_fbi = info->par;
	struct mxcfb_info *fg_mxc_fbi = NULL;

	/* swap only happen between DP-BG and DC, while DP-FG disable */
	if (((mxc_fbi->ipu_ch == MEM_BG_SYNC) &&
	     (strstr(buf, "1-layer-fb") != NULL)) ||
	    ((mxc_fbi->ipu_ch == MEM_DC_SYNC) &&
	     (strstr(buf, "2-layer-fb-bg") != NULL))) {
		int i;

		for (i = 0; i < num_registered_fb; i++) {
			fg_mxc_fbi = mxcfb_info[i]->par;
			if (fg_mxc_fbi->ipu_ch == MEM_FG_SYNC)
				break;
			else
				fg_mxc_fbi = NULL;
		}
		if (!fg_mxc_fbi ||
			fg_mxc_fbi->blank == FB_BLANK_UNBLANK) {
			dev_err(dev,
 ...
 */
static int __devinit mxcfb_probe(struct platform_device *pdev)
{
	struct fb_info *fbi;
	struct mxcfb_info *mxc_fbi;
	...
	if (!ipu_init_ok()) {
		printk(KERN_ERR "%s: IPU not initialized\n", __FUNCTION__);
		return -ENODEV;
	}
	...
	if (!g_dp_in_use) {
		mxc_fbi->ipu_ch_irq = IPU_IRQ_BG_SYNC_EOF;
		mxc_fbi->ipu_ch = MEM_BG_SYNC;
		mxc_fbi->blank = FB_BLANK_UNBLANK;
	} else {
		mxc_fbi->ipu_ch_irq = IPU_IRQ_DC_SYNC_EOF;
		mxc_fbi->ipu_ch = MEM_DC_SYNC;
	...
	if (pdev->id == 0) {
		ipu_disp_set_global_alpha(mxc_fbi->ipu_ch, 1, 0x80);
		ipu_disp_set_color_key(mxc_fbi->ipu_ch, 0, 0);
		strcpy(fbi->fix.id, "DISP3 BG");

		if (!g_dp_in_use) {
			ret = ipu_request_irq(IPU_IRQ_BG_ALPHA_SYNC_EOF,
					mxcfb_alpha_irq_handler, 0,
					MXCFB_NAME, fbi);
			if (ret) {
				dev_err(&pdev->dev, "Error registering BG alpha irq handler\n");
				goto err1;
			}
			g_dp_in_use = 1;
		}
		...
		mxc_fbi->ipu_ch_irq = IPU_IRQ_FG_SYNC_EOF;
		mxc_fbi->ipu_ch = MEM_FG_SYNC;
		mxc_fbi->ipu_di = -1;
		mxc_fbi->overlay = 1;
		mxc_fbi->blank = FB_BLANK_POWERDOWN;
		...
		ret = ipu_request_irq(IPU_IRQ_FG_ALPHA_SYNC_EOF,
				mxcfb_alpha_irq_handler, 0,
				MXCFB_NAME, fbi);
		if (ret) {
			dev_err(&pdev->dev, "Error registering FG alpha irq handler\n");
			goto err1;
		}
	}
	...
	ret = ipu_request_irq(mxc_fbi->ipu_ch_irq, mxcfb_irq_handler, 0,
			MXCFB_NAME, fbi);
	if (ret) {
	...
	if (res) {
		fbi->fix.smem_len = resource_size(res);
	...
	if (!fb_mode && plat_data && plat_data->mode_str) {
		DBG(0, "%s: Searching for video mode '%s'\n", __FUNCTION__,
			plat_data->mode_str);
		if (!fb_find_mode(&fbi->var, fbi, plat_data->mode_str, NULL, 0, NULL,
					default_bpp)) {
			DBG(0, "%s: Not found\n", __FUNCTION__);
		} else {
			struct fb_var_screeninfo *var = &fbi->var;

			DBG(0, "%s: Found: %dx%d-%d HS:%d:%d:%d VS:%d:%d:%d\n",
				__FUNCTION__,
				var->xres, var->yres, var->bits_per_pixel,
				var->hsync_len, var->left_margin, var->right_margin,
				var->vsync_len, var->upper_margin, var->lower_margin);
	...
	if (fb_mode) {
		DBG(0, "%s: Searching for video mode '%s'\n", __FUNCTION__,
			fb_mode);
		if (!fb_find_mode(&fbi->var, fbi, fb_mode, NULL, 0, NULL,
					default_bpp)) {
			DBG(0, "%s: Not found\n", __FUNCTION__);
		} else {
			struct fb_var_screeninfo *var = &fbi->var;
	...
	if (plat_data) {
		...
		if (!fb_mode && plat_data->mode) {
			...
			DBG(0, "%s: Using video mode '%s'(%p) from platform data\n",
				__FUNCTION__, plat_data->mode_str, plat_data->mode);
			fb_videomode_to_var(&fbi->var, plat_data->mode);

			DBG(0, "%s: %dx%d-%d HS:%d:%d:%d VS:%d:%d:%d\n",
				__FUNCTION__,
				var->xres, var->yres, var->bits_per_pixel,
				var->hsync_len, var->left_margin, var->right_margin,
				var->vsync_len, var->upper_margin, var->lower_margin);
	...
	fbi->var.xres_virtual = max_t(int, def_vxres, fbi->var.xres);
	fbi->var.yres_virtual = max_t(int, def_vyres, fbi->var.yres);
	DBG(0, "%s: Screen: %ux%u (%ux%u)\n", __FUNCTION__,
		fbi->var.xres, fbi->var.yres,
	...
	printk(KERN_INFO "mxc_ipuv3_fb: %dx%d-%d\n",
		fbi->var.xres, fbi->var.yres, fbi->var.bits_per_pixel);
...
static int __devexit mxcfb_remove(struct platform_device *pdev)
{
 ...
 */
static int mxcfb_suspend(struct device *dev)
{
 ...
 */
static int mxcfb_resume(struct device *dev)
{
	struct fb_info *fbi = dev_get_drvdata(dev);
	struct mxcfb_info *mxc_fbi = fbi->par;
	int saved_blank;
 ...
 */
static struct dev_pm_ops mxcfb_pm_ops = {
...
static struct platform_driver mxcfb_driver = {
	.driver = {
		.name = MXCFB_NAME,
		.pm = __dev_pm_ops_p(mxcfb_pm_ops),
	},
	.probe = mxcfb_probe,
 ...
 * example:
 *	video=mxcfb:800x600,bpp=16,noaccel
 */
