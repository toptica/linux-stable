	...
	case IPU_INIT_CHANNEL:
		{
			...
			if (copy_from_user(&parm, (ipu_channel_parm *)arg,
						sizeof(ipu_channel_parm)))
				return -EFAULT;

			if (!parm.flag) {
				ret = ipu_init_channel(parm.channel,
						&parm.params);
			} else {
	...
	case IPU_INIT_CHANNEL_BUFFER:
		{
			...
			if (copy_from_user(&parm, (ipu_channel_buf_parm *)arg,
	...
	case IPU_SELECT_CHANNEL_BUFFER:
		{
			...
			ret = ipu_select_buffer(parm.channel,
	...
	case IPU_LINK_CHANNELS:
		{
			...
			if (copy_from_user(&link, (ipu_channel_link *)arg,
	...
	case IPU_DISABLE_CHANNEL:
		{
			...
			if (copy_from_user(&info, (ipu_channel_info *)arg,
	...
	case IPU_FREE_IRQ:
		{
			...
			if (copy_from_user(&info, (ipu_irq_info *)arg,
						sizeof(ipu_irq_info)))
				return -EFAULT;
	...
	case IPU_REGISTER_GENERIC_ISR:
		{
			...
			if (copy_from_user(&info, (ipu_event_info *)arg,
						sizeof(ipu_event_info)))
				return -EFAULT;

			ret = ipu_request_irq(info.irq,
	...
	case IPU_ALOC_MEM:
		{
			...
			if (copy_from_user(&info, (ipu_mem_info *)arg,
						sizeof(ipu_mem_info)))
				return -EFAULT;
	...
	case IPU_IS_CHAN_BUSY:
		{
			...
			if (copy_from_user(&chan, (ipu_channel_t *)arg,
