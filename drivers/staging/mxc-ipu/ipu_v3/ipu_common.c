...
static uint32_t ipu_bytes_per_pixel(uint32_t fmt);
...
static void ipu_unmap_registers(void)
{
	iounmap(ipu_cm_reg);
	iounmap(ipu_ic_reg);
	iounmap(ipu_idmac_reg);
...
static int __init ipu_probe(struct platform_device *pdev)
{
	...
	ret = ipu_map_registers(res->start);
	if (ret) {
		return ret;
	}

	if (g_ipu_hw_rev == 2) {
		/* The MIPI HSC unit has been removed from the
		 * i.MX51 Reference Manual by the Freescale marketing division.
		 * However this did not remove the hardware from the chip
		 * which still needs to be configured...
		 */
		struct clk *hsc_clk = clk_get(NULL, "mipi_hsp");
		void __iomem *hsc_addr = ioremap(MIPI_HSC_BASE_ADDR, PAGE_SIZE);
		...
		if (IS_ERR(hsc_clk)) {
			printk(KERN_ERR "%s: Failed to get HSC clock: %d\n", __FUNCTION__,
				(int)PTR_ERR(hsc_clk));
			ret = PTR_ERR(hsc_clk);
			iounmap(hsc_addr);
			goto unmap;
...
#if 0
	/* hard reset the IPU */
	__raw_writel(__raw_readl(MX5_IO_ADDRESS(SRC_BASE_ADDR)) |
		(1 << 3),
		MX5_IO_ADDRESS(SRC_BASE_ADDR));
#endif
	/* Enable IPU and CSI clocks */
	/* Get IPU clock freq */
	g_ipu_clk = clk_get(&pdev->dev, "ipu");
	if (IS_ERR(g_ipu_clk)) {
		dev_err(&pdev->dev, "%s: Failed to get ipu_clk\n", __FUNCTION__);
		ret = PTR_ERR(g_ipu_clk);
		goto unmap;
	}
	...
	g_csi_clk[0] = clk_get(NULL, "csi0_mclk");
	if (IS_ERR(g_csi_clk[0])) {
		dev_err(&pdev->dev, "%s: Failed to get csi0_mclk\n", __FUNCTION__);
		ret = PTR_ERR(g_csi_clk[0]);
		goto put_ipu_clk;
	}

	g_csi_clk[1] = clk_get(NULL, "csi1_mclk");
	if (IS_ERR(g_csi_clk[1])) {
		dev_err(&pdev->dev, "%s: Failed to get csi1_mclk\n", __FUNCTION__);
		ret = PTR_ERR(g_csi_clk[1]);
		goto put_mclk1;
	}

	ret = request_irq(g_ipu_irq[0], ipu_irq_handler, IRQF_DISABLED,
			pdev->name, &pdev->dev);
	if (ret) {
		dev_err(g_ipu_dev, "request SYNC interrupt failed: %d\n", ret);
		goto put_mclk2;
	}
	/* Some platforms have 2 IPU interrupts */
	g_ipu_irq[1] = platform_get_irq(pdev, 1);
	if (g_ipu_irq[1] >= 0) {
		ret = request_irq(g_ipu_irq[1], ipu_irq_handler, IRQF_DISABLED,
				pdev->name, &pdev->dev);
		if (ret) {
			dev_err(g_ipu_dev, "request ERR interrupt %d failed: %d\n",
				g_ipu_irq[1], ret);
			goto free_irq1;
		}
	}
	...
	__raw_writel(__raw_readl(MX5_IO_ADDRESS(SRC_BASE_ADDR)) |
		(1 << 3),
		MX5_IO_ADDRESS(SRC_BASE_ADDR));
	dev_dbg(&pdev->dev, "Asserting IPU RESET\n");
	...
	while (__raw_readl(IPU_MEM_RST) & 0x80000000) {
		...
		if (ret <= 0) {
			dev_err(&pdev->dev,
				"Wait for IPU reset timed out\n");
			ret = -ETIME;
			goto free_irq2;
		}
		udelay(100);
	}
	dev_dbg(&pdev->dev, "IPU RESET done\n");
...
free_irq2:
	if (g_ipu_irq[1] >= 0)
		free_irq(g_ipu_irq[1], &pdev->dev);
free_irq1:
	free_irq(g_ipu_irq[0], &pdev->dev);
put_mclk2:
	clk_put(g_csi_clk[1]);
put_mclk1:
	clk_put(g_csi_clk[0]);
 ...
 */
int ipu_init_channel(ipu_channel_t channel, ipu_channel_params_t *params)
{
	int ret = 0;
	uint32_t ipu_conf;
	uint32_t reg;
...
static int _ipu_ch_param_init(int ch,
			...
			dma_addr_t addr1)
{
	...
	case IPU_PIX_FMT_NV12:
		...
		break;
	default:
		dev_err(g_ipu_dev, "mxc ipu: unimplemented pixel format: %d\n",
			pixel_fmt);
		return -EINVAL;
	}
 ...
 * @return      Returns 0 on success or negative error code on fail
 */
int ipu_init_channel_buffer(ipu_channel_t channel, ipu_buffer_t type,
				uint32_t pixel_fmt,
				...
				uint32_t u, uint32_t v)
{
	...
	ret = _ipu_ch_param_init(dma_chan, pixel_fmt, width, height, stride, u, v, 0,
			   phyaddr_0, phyaddr_1);
	if (ret)
		return ret;
 ...
 */
int ipu_update_channel_buffer(ipu_channel_t channel, ipu_buffer_t type,
 ...
 * @return      Returns 0 on success or negative error code on fail
 */
int ipu_select_buffer(ipu_channel_t channel, ipu_buffer_t type,
			  uint32_t bufNum)
{
	uint32_t dma_chan = channel_2_dma(channel, type);
 ...
 * @return      Returns 0 on success or negative error code on fail
 */
int ipu_select_multi_vdi_buffer(uint32_t bufNum)
{
 ...
 *              fail.
 */
int ipu_link_channels(ipu_channel_t src_ch, ipu_channel_t dest_ch)
{
	int retval = 0;
	unsigned long flags;
 ...
 *              fail.
 */
int ipu_unlink_channels(ipu_channel_t src_ch, ipu_channel_t dest_ch)
{
	int retval = 0;
	unsigned long flags;
	uint32_t fs_proc_flow1;
 ...
 *              0 for not enabled.
 */
int ipu_is_channel_busy(ipu_channel_t channel)
{
	uint32_t reg;
	uint32_t in_dma;
	uint32_t out_dma;
 ...
 *              fail.
 */
int ipu_enable_channel(ipu_channel_t channel)
{
	uint32_t reg;
	unsigned long flags;
	uint32_t in_dma;
 ...
 *              fail.
 */
int ipu_disable_channel(ipu_channel_t channel, bool wait_for_stop)
{
	uint32_t reg;
	unsigned long flags;
	uint32_t in_dma;
...
static irqreturn_t ipu_irq_handler(int irq, void *desc)
{
	...
	uint32_t int_stat;
	const int err_reg[] = { 5, 6, 9, 10, };
	const int int_reg[] = { 1, 2, 3, 4, 11, 12, 13, 14, 15, };
#if 1
	static int running;
...
#else
	if (g_ipu_irq[1] >= 0) {
		disable_irq_nosync(g_ipu_irq[0]);
		disable_irq_nosync(g_ipu_irq[1]);
	}
#endif
	for (i = 0; i < ARRAY_SIZE(err_reg); i++) {
	...
	for (i = 0; i < ARRAY_SIZE(int_reg); i++) {
		...
		while ((line = ffs(int_stat)) != 0) {
			...
			int_stat &= ~(1UL << line);
			line += (int_reg[i] - 1) * 32;
			result |= ipu_irq_list[line].handler(line,
							ipu_irq_list[line].
							dev_id);
		}
	}
...
int ipu_swap_channel(ipu_channel_t from_ch, ipu_channel_t to_ch)
{
	uint32_t reg;
	unsigned long flags;
...
//EXPORT_SYMBOL(ipu_bytes_per_pixel);
...
static int ipu_suspend(struct platform_device *pdev, pm_message_t state)
{
	if (g_ipu_clk_enabled) {
		...
		while ((__raw_readl(IDMAC_CHA_BUSY(0)) & idma_enable_reg[0]) ||
			...
			chan_should_disable = __raw_readl(IDMAC_CHA_BUSY(0)) ^
				idma_enable_reg[0];
			__raw_writel((~chan_should_disable) &
					idma_enable_reg[0], IDMAC_CHA_EN(0));
			chan_should_disable = __raw_readl(IDMAC_CHA_BUSY(1)) ^
				idma_enable_reg[1];
			__raw_writel((~chan_should_disable) &
