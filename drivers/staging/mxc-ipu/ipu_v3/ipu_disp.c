...
/* Pls keep S0, S1 and S2 as 0x2 by using this conversion */
			    ...
			    uint32_t v_to_h_sync, ipu_di_signal_cfg_t sig)
{
	...
	dev_dbg(g_ipu_dev, "disp %d: panel size = %d x %d\n",
		disp, width, height);
	dev_dbg(g_ipu_dev, "margins: SYNC: %d+%d HOR: %d+%d VERT: %d+%d\n",
		h_sync_width, v_sync_width, h_start_width, h_end_width,
		v_start_width, v_end_width);

	if (disp >= ARRAY_SIZE(g_di_clk))
		return -EINVAL;
