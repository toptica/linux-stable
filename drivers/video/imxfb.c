...
static const struct imx_fb_videomode *imxfb_find_mode(struct imxfb_info *fbi)
{
	...
	if (!fb_mode)
		return NULL;
	printk(KERN_DEBUG "%s: looking up mode '%s'\n", __FUNCTION__, fb_mode);
	for (i = 0, m = &fbi->mode[0]; i < fbi->num_modes; i++, m++) {
		printk(KERN_DEBUG "%s: mode[%d]='%s'\n", __FUNCTION__, i, m->mode.name);
...
static int imxfb_check_var(struct fb_var_screeninfo *var, struct fb_info *info)
{
	...
	printk("Setting PCD to %u for LCD clock %u.%03uMHz base clock %lu.%03luMHz actual clock %lu.%03luMHz\n",
