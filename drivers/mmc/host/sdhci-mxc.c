...
static void sdhci_send_command(struct sdhci_host *host, struct mmc_command *cmd)
{
	...
	DBG(2, "sdhci_send_command 0x%08x is starting... cmd=%p data=%p data_early=%d\n",
		cmd->opcode, host->cmd, host->data, host->data_early);
	WARN_ON(host->cmd && !host->data_early);
...
no_detect_irq:
	...
	device_set_wakeup_enable(mmc_dev(mmc), 1);
	return 0;
