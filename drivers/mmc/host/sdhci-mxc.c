...
static void sdhci_prepare_data(struct sdhci_host *host, struct mmc_data *data)
{
	...
	if (host->flags & SDHCI_REQ_USE_DMA) {
		...
		for (i = 0; i < count; i++, tsg++) {
...
#else
			unsigned int *adma_desc = &host->adma_des_table[2 * i];
			...
			*adma_desc = (tsg->length << 12) |
				FSL_ADMA_DES_ATTR_SET |
				FSL_ADMA_DES_ATTR_VALID;
			adma_desc++;
			*adma_desc = tsg->dma_address |
				FSL_ADMA_DES_ATTR_TRAN |
				FSL_ADMA_DES_ATTR_VALID |
				((count == (i + 1)) ? FSL_ADMA_DES_ATTR_END : 0);
			DBG(2, "%s: addr[%d] %08x len %08x %s\n",
				__FUNCTION__, i, *adma_desc, adma_desc[-1],
				*adma_desc & FSL_ADMA_DES_ATTR_END ?
				"last" : "");
#endif
		}
...
static int sdhci_suspend(struct device *dev)
{
	...
	for (i = 0; i < chip->num_slots; i++) {
		if (!chip->hosts[i])
			continue;
		ret = mmc_suspend_host(chip->hosts[i]->mmc, PMSG_SUSPEND);
		if (ret) {
			for (i--; i >= 0; i--)
...
static int sdhci_resume(struct device *dev)
{
	...
	for (i = 0; i < chip->num_slots; i++) {
		if (!chip->hosts[i])
			continue;
		ret = request_irq(chip->hosts[i]->irq, sdhci_irq, 0,
...
no_detect_irq:
	...
	if (host->flags & SDHCI_USE_DMA) {
		...
		host->adma_des_table = dma_alloc_coherent(&pdev->dev,
							...
							GFP_DMA);
#else
		host->adma_des_table = kcalloc((2 * (mmc->max_phys_segs) + 1),
