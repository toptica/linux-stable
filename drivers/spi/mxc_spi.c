				      ...
				      unsigned int baud)
{
	...
	/* get the next higher even divisor */
	shift = 2;
	while ((1 << (shift / 2 + 1)) + ((shift & 1) << (shift / 2)) < divisor)
		shift++;
#else
	while (divisor >>= 1)
		shift++;
...
static int mxc_spi_probe(struct platform_device *pdev)
{
	...
	if (mxc_platform_info->init) {
		ret = mxc_platform_info->init(pdev);
		if (ret) {
			dev_err(&pdev->dev, "platform_init failed with error: %d\n", ret);
			goto err2;
		}
	}
	...
	ret = clk_enable(master_drv_data->clk);
	if (ret) {
		pr_err("%s: Failed to enable cspi_clk\n", __FUNCTION__);
		goto err3;
	}
