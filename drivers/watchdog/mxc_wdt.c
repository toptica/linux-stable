...
static void mxc_wdt_ping(void __iomem *base)
{
...
static void mxc_wdt_config(void __iomem *base)
{
	u16 val;
...
static void mxc_wdt_enable(void __iomem *base)
{
	u16 val;
...
static void mxc_wdt_disable(void __iomem *base)
{
...
static u16 mxc_wdt_get_timeout(void __iomem *base)
{
	u16 val;
...
static u16 mxc_wdt_get_bootreason(void __iomem *base)
{
	u16 val;
...
static void mxc_wdt_set_timeout(void __iomem *base)
{
	u16 val;
	val = __raw_readw(base + MXC_WDT_WCR);
...
static int __devinit mxc_wdt_probe(struct platform_device *pdev)
{
	...
	wdt_base_reg = ioremap(res->start, resource_size(res));
	if (wdt_base_reg == NULL) {
		ret = -ENOMEM;
		goto release;
	}
	...
	mxc_wdt_clk = clk_get(&pdev->dev, "wdog_clk");
...
static int mxc_wdt_suspend(struct device *dev)
{
	if (mxc_wdt_users) {
		mxc_wdt_disable(wdt_base_reg);
	}
...
static int mxc_wdt_resume(struct device *dev)
{
	if (mxc_wdt_users) {
		mxc_wdt_enable(wdt_base_reg);
		mxc_wdt_ping(wdt_base_reg);
...
static struct dev_pm_ops mxc_wdt_pm_ops = {
