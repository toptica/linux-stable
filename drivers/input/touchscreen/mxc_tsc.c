...
struct mxc_tsc_adc_data {
	struct mxc_tsc_fifo_entry data[8];
};
...
static int mxc_tsc_read_adc(struct mxc_tsc *ts_dev, int chan)
{
	...
	if (chan<0) {
		...
		mxc_tsc_clr_mask(ts_dev, GCQCR, CQCR_LAST_ITEM_ID_MASK | CQCR_FIFOWATERMARK_MASK | CQCR_QSM_MASK);
		mxc_tsc_set_mask(ts_dev, GCQCR, 0 << CQCR_LAST_ITEM_ID_SHIFT | (0 << CQCR_FIFOWATERMARK_SHIFT));
	}


	memset(adc_data, 0, sizeof(*adc_data));
	irqbuf->irqcount = 0;
	irqbuf->reqcount = (chan < 0)?8 : 1;
...
MXC_TSC_DEV_ATTR(adc0, S_IRUGO, CC_SELIN_XPUL, mxc_tsc_attr_get);
MXC_TSC_DEV_ATTR(adc1, S_IRUGO, CC_SELIN_YPLL, mxc_tsc_attr_get);
MXC_TSC_DEV_ATTR(adc2, S_IRUGO, CC_SELIN_XNUR, mxc_tsc_attr_get);
MXC_TSC_DEV_ATTR(adc3, S_IRUGO, CC_SELIN_YNLR, mxc_tsc_attr_get);
...
MXC_TSC_DEV_ATTR(all, S_IRUGO, -1, mxc_tsc_attr_getall);
	...
	*/
	...
	for (i=0; i < 8; i++) {
		reg = (0 << CC_NOS_SHIFT) |
			...
			(intref * CC_SEL_REFP_INT) | CC_SEL_REFN_AGND |
			(i << CC_SELIN_SHIFT);
	 ...
	 * but the kernel, spending time on other thing and therefore
