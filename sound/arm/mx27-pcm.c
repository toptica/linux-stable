...
static const struct snd_pcm_hardware mx27_pcm_hardware = {
	...
	.periods_min		= 2,
	.periods_max		= PAGE_SIZE / sizeof(struct imx_dma_desc),
...
struct mx27_runtime_data {
	...
	struct imx_dma_desc *dma_desc_array;
	struct sg_table sg;
	int periods;
	int period;
};
			...
			struct snd_pcm_substream *substream)
{
	...
	if (rtd->sg.sgl) {
		if (playback) {
			ret = imx_dma_setup_sg(rtd->dma_ch,
		...
		struct imx_dma_desc *dma_desc =
			&rtd->dma_desc_array[rtd->period];

		if (playback) {
			ret = imx_dma_setup_single(rtd->dma_ch,
						dma_desc->src_addr,
...
static void mx27_pcm_sg_callback(int dma_ch, void *arg, struct scatterlist *sg)
{
	...
	DBG(0, "%s: %s period %u/%u sg: %p next %p\n", __FUNCTION__,
				...
				struct snd_pcm_hw_params *params)
{
	...
	WARN_ON(totsize != 0);
	BUG_ON(sg == NULL);
	/* chain back the last entry to the head of the sg list */
...
static int mx27_pcm_open(struct snd_pcm_substream *substream)
{
	...
	ret = imx_dma_request_by_prio(rtd->dma_params->name, DMA_PRIO_HIGH);
	if (ret < 0) {
		goto free_mem;
	}
	rtd->dma_ch = ret;
	DBG(0, "%s: DMA channel %d allocated to %d\n", __FUNCTION__,
		rtd->dma_params->channel_id, ret);

	imx_dma_config_burstlen(rtd->dma_ch, playback ?
				TXFIFO_WATERMARK :
				RXFIFO_WATERMARK);
	ret = imx_dma_config_channel(rtd->dma_ch,
	...
	ret = imx_dma_setup_handlers(rtd->dma_ch,
	...
	ret = imx_dma_setup_progression_handler(rtd->dma_ch,
...
int mx27_pcm_new(struct snd_card *card, struct mxc_pcm_client *client,
		 struct snd_pcm **rpcm)
{
	struct snd_pcm *pcm;
