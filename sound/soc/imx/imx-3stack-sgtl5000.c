 ...
 *    2010-08-04:     adapted for 2.6.31 by <LW@KARO-electronics.de>
 *
 */
				      ...
				      struct snd_pcm_hw_params *params)
{
	...
	if ((asrc_ssi_data.output_sample_rate != 0) &&
		...
		struct mxc_runtime_data *pcm_data = substream->runtime->private_data;
		struct asrc_config config;
		struct mxc_audio_platform_data *plat;
		struct imx_3stack_priv *priv = &card_priv;
		int ret = 0;
		ret = asrc_req_pair(channel, &asrc_ssi_data.asrc_index);
		if (ret < 0) {
			pr_err("asrc_req_pair failed: %d\n", ret);
			return ret;
		}
...
static int imx_3stack_init_dam(int ssi_port, int dai_port)
{
	int ret;

	/* SGTL5000 uses SSI1 or SSI2 via AUDMUX port dai_port for audio */
	ret = mxc_audmux_configure_sync_slave(ssi_port, dai_port, 0);
	if (ret) {
		printk(KERN_ERR "Failed to configure audmux: %d\n", ret);
	}
	return 0;
}
...
static int __devinit imx_3stack_sgtl5000_probe(struct platform_device *pdev)
{
	...
	ret = imx_3stack_init_dam(plat->src_port, plat->ext_port);
	if (ret)
		return ret;
...
MODULE_LICENSE("GPL");
MODULE_ALIAS("platform:imx-3stack-sgtl5000");
